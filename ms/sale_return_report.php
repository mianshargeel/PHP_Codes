     ...
     <a href="javascript:showCal('Calendar2')">
      <input type="text" style="width:40%; float:left; margin-left:1%;" class="form-control" name="secondinput" placeholder="to" />
		...
		if(isset($_GET['submit'])){
		 ...
		 $qryy	=	mysql_query("select * from tbl_bills_no where date_search like '%$search' && sr = 1 ") or die(mysql_error());
		...
		$tt_mount += $kk['total_bill']; }
		  ...
		  $qry	=	mysql_query("select * from tbl_bills_no where date_search like '%$search' && sr = 1 ") or die(mysql_error());
